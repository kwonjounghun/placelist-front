...
export function GetMyListRequest(token) {
    return (dispatch) => {
        dispatch(GetMyList());

        return axios.get("http://localhost:7777/api/place/getList", { headers: { 'x-access-token': token, 'Content-Type': 'application/json' } })
            .then((response) => {
